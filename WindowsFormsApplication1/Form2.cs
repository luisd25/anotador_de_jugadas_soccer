...
namespace WindowsFormsApplication1
{
    ...
    public partial class Form2 : Form
    {
        ...
        private void accion_Click(object sender, EventArgs e)
        {
            ...
            else if (pases.Enabled == true) { x = 2; jugada = pases.SelectedItem.ToString(); aux = JugadorT1_2.SelectedItem.ToString(); }
            else if(jugadas.Enabled==true){x=3;jugada=jugadas.Text;}
            else if(faltas.Enabled==true){x=4;jugada=faltas.SelectedItem.ToString();}
            else if(saques.Enabled==true){x=5;jugada=saques.SelectedItem.ToString();}
            
            if (Team1.Checked == true) equipo = label1.Text;
            else if (Team2.Checked == true) equipo = label2.Text;
            
            reg.registrar(JugadorT1.SelectedItem.ToString(),aux,x,lblMin.Text+":"+lblSeg.Text,jugada,equipo);
            
            if (x==1)
            if (MessageBox.Show("¿Anotó con el tiro?", "Tiro Libre", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                
                /*int gol = Convert.ToInt32(textBox1.Text);
                gol += 1;
